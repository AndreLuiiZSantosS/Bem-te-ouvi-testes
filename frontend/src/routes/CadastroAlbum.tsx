...
export default function Pagina1CriarAlbum() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const musicianIds = selectedMusicians.map((m) => m.id);
    formData.append("musicos", JSON.stringify(musicianIds));
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Falha ao criar álbum. Tente novamente.");
      }

      const newAlbum = await response.json();
      navigate(`/criarAlbum/2/${newAlbum.id}`);
    } catch (err: any) {
  ...
  return (
    <div className="mb-[75px]"> {/* 50px de diff entre o player o botão */}
      ...
      >
        ...
        <div>
          <ImageUpload onImageChange={(file) => setImagemCapa(file)} />
        </div>
            ...
            {selectedMusicians.map((musician) => (
              ...
              >
                ...
                <button
                  ...
                  aria-label={`Remover ${musician.name}`}
                >
                  &times;
                </button>
              </span>
        ...
        <div className="self-end">
          <Button
            text={isLoading ? "Enviando..." : "Avançar"}
            variant="proximo"
            type="submit"
            onClick={() => navigate('/criarAlbum/2/${albumId}')}
          />
        </div>
      </form>
    </div>
